...
export const TrialBanner: React.FC = () => {
  ...
  if (!currentUser || !trialInfo.isTrial) {
    return null;
  }
    ...
    return (
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span>
            <strong>Free Trial Expired:</strong> Your 4-day trial has ended. Upgrade to continue publishing and managing your store.
          </span>
        </div>
        <button
          id="btn-trial-upgrade-expired"
          onClick={() => navigateTo('pricing')}
          className="bg-white text-rose-700 hover:bg-rose-50 px-3.5 py-1 rounded-xl font-black text-xs shrink-0 flex items-center gap-1 transition-all shadow-xs"
        >
    ...
    >
      <div className="flex items-center gap-2.5">
        ...
        <span className="flex items-center gap-2 flex-wrap">
          ...
          <span className="text-blue-100">{trialInfo.formattedRemaining} (Ends {trialInfo.expiryDateFormatted})</span>
          <span className="hidden sm:inline-block bg-white/20 px-2 py-0.5 rounded-md text-[11px] font-mono">
            {products.length} / {trialInfo.maxProducts} products
          </span>
        </span>
      </div>
      <button
        ...
        onClick={() => navigateTo('pricing')}
        className="bg-white/15 hover:bg-white/25 text-white border border-white/30 px-3 py-1 rounded-xl font-bold text-xs shrink-0 flex items-center gap-1.5 transition-all"
      >
