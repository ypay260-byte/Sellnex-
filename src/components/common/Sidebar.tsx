...
export const Sidebar: React.FC<SidebarProps> = ({ mobileOpen, setMobileOpen }) => {
  ...
  const storeMenuItems: SidebarMenuItem[] = [
  ...
  const restaurantMenuItems: SidebarMenuItem[] = [
    { id: 'restaurant-dashboard', label: t('nav_orders', 'Buyurtmalar'), icon: ShoppingBag, highlight: true },
    { id: 'restaurant-menu', label: t('nav_products', 'Menyu & Mahsulotlar'), icon: Utensils },
    { id: 'restaurant-settings', label: t('nav_settings', 'Sozlamalar'), icon: Settings },
    ...
    <>
      ...
      <div className="p-3 border-b border-slate-100 shrink-0 space-y-2">
        <div className={`rounded-xl p-3 border space-y-2 ${isRestaurant ? 'bg-amber-50/60 border-amber-200/80' : 'bg-slate-50 border-slate-200/80'}`}>
          <div className="flex items-center justify-between">
            ...
            <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${isRestaurant ? 'bg-amber-100 text-amber-800' : 'bg-emerald-100 text-emerald-700'}`}>
              {isRestaurant ? 'Restoran' : t('sidebar_live_badge', 'Live')}
            </span>
          </div>
          ...
          <div className="grid grid-cols-2 gap-1.5 pt-1">
            <button
              ...
              className="flex items-center justify-center gap-1 py-1.5 px-2 rounded-lg bg-white border border-slate-200 hover:bg-slate-100 text-slate-700 text-[11px] font-bold transition-colors group cursor-pointer"
            >
              <span>{isRestaurant ? 'Menyu' : t('sidebar_preview', 'Preview')}</span>
            ...
            <button
              ...
              className={`flex items-center justify-center gap-1 py-1.5 px-2 rounded-lg border text-[11px] font-bold transition-colors cursor-pointer ${
                isRestaurant
            ...
            >
              ...
              <span>{t('sidebar_copy_link', 'Copy Link')}</span>
            </button>
          </div>
        </div>
        ...
        >
          ...
          <div className="flex items-center justify-between text-[9px] text-blue-800/80 font-medium">
            <span>{t('sidebar_limit_used', 'Product limit used')}</span>
            <span className="font-bold underline text-blue-700">{t('sidebar_upgrade', 'Upgrade →')}</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-1 custom-scrollbar">
        ...
        <div className="pt-4 text-[10px] font-bold uppercase tracking-widest text-slate-400 px-3">
          {t('sidebar_integrations_title', 'Integrations')}
        </div>
        ...
        <div className="pt-3 mt-3 border-t border-slate-100">
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span>{isRestaurant ? t('sidebar_switch_to_store', '🛍️ Do‘kon Rejimiga o‘tish') : t('sidebar_switch_to_cafe', '🍽️ Restoran Rejimiga o‘tish')}</span>
            </div>
            <span className="text-[10px] opacity-75 font-semibold">{t('sidebar_change_mode', 'O‘zgartirish')}</span>
          </button>
        </div>
