...
export const AuthPage: React.FC = () => {
  ...
  const loadDemoAccount = (accEmail: string, accPass: string) => {
    setEmail(accEmail);
    setPassword(accPass);
    setMode('login');
    setErrorMsg('');
  ...
  return (
    <div id="auth-page-root" className="min-h-screen bg-slate-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md px-4">
        ...
        {/* Optional Demo Accounts Accordion for Testing */}
        <div className="mt-4 border border-slate-200 bg-white/70 backdrop-blur rounded-xl p-3 text-xs text-slate-500">
          <p className="font-semibold text-slate-700 mb-1.5 flex items-center gap-1">
            <KeyRound className="w-3.5 h-3.5 text-slate-400" />
            <span>Developer / Demo Testing Shortcuts:</span>
          </p>
          <div className="flex flex-wrap gap-1.5">
            <button
              onClick={() => loadDemoAccount('demo@sellnex.uz', 'demo123')}
              className="px-2 py-1 bg-slate-100 hover:bg-slate-200 rounded text-[11px] text-slate-700 font-mono"
            >
              demo@sellnex.uz
            </button>
            <button
              onClick={() => loadDemoAccount('admin@sellnex.uz', 'admin123')}
              className="px-2 py-1 bg-slate-100 hover:bg-slate-200 rounded text-[11px] text-slate-700 font-mono"
            >
              admin@sellnex.uz
            </button>
